...
import { Plus, Flame, Clock, Search, Zap, CheckCircle2, X } from 'lucide-react';
...
export default function MarketPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      const res  = await fetch(`/api/tasks?${params}&t=${Date.now()}`, { cache: 'no-store' });
      ...
      console.error('[market] fetchTasks error:', e);
    }
    setLoading(false);
  ...
  // Re-fetch when user navigates back to this page (Next.js 14 router cache fix)
  useEffect(() => {
    const onFocus = () => void fetchTasks();
  ...
  const handleTaskCreated = (taskId: string) => {
    setShowModal(false);
    router.refresh(); // Invalidate Next.js router cache so /market re-fetches on back
    router.push(`/market/${taskId}?autorun=1`);
  };
  ...
  return (
    <>
      ...
      <div className="pt-14 min-h-screen">
        ...
        {/* Filters */}
        <div className="border-b border-acid/10 px-5 md:px-14 py-4 flex flex-col gap-3">
          {/* Search + sort row */}
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-48 max-w-sm">
              ...
              <input
                ...
                className="w-full bg-dim border border-acid/15 text-white font-mono text-xs pl-8 pr-10 py-2 focus:outline-none focus:border-acid/40 placeholder:text-muted transition-colors"
              />
              {search && (
            ...
            <span className="font-mono text-xs text-muted/30 hidden md:block">{tasks.length} tasks</span>
          </div>

          {/* Filter chips row */}
          <div className="flex flex-wrap gap-2 items-center">
            <span className="font-mono text-xs text-muted/30 mr-1">CAT:</span>
            ...
            <div className="w-px h-4 bg-acid/15 mx-1" />
            <span className="font-mono text-xs text-muted/30 mr-1">STATUS:</span>
            {['All', 'open', 'running', 'completed'].map(s => {
              const color = STATUS_META[s]?.color;
              return (
                <button key={s} onClick={() => setStatus(s)}
                  className={`filter-chip ${statusFilter === s ? 'active' : ''}`}
                  style={statusFilter === s && color ? { borderColor: color, color } : {}}>
                  {s === 'All' ? 'ALL' : (
                    <span className="flex items-center gap-1.5">
                      {ACTIVE_STATUSES.has(s) && <span className="w-1 h-1 rounded-full animate-pulse" style={{ background: color }} />}
                      {s.toUpperCase()}
                    </span>
                  )}
                </button>
