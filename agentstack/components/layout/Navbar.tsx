          ...
          <div className="hidden md:flex items-center gap-1 flex-1 justify-center">
            {NAV.map(({ href, label }) => (
              <Link key={href} href={href}
                className={cn('font-mono text-xs tracking-widest px-4 py-2 transition-all',
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden pt-14" style={{ background: 'rgba(6,6,8,0.97)' }}>
          <div className="flex flex-col border-t border-acid/15">
            {NAV.map(({ href, label }) => (
              <Link key={href} href={href}
                className={cn('font-mono text-sm tracking-widest px-6 py-5 border-b border-acid/10 transition-colors',
