...
import { Flame, Trophy, Clock, Plus, Zap, CheckCircle2, RotateCcw, ExternalLink, Share2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const [runningAgain, setRunningAgain] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    if (address) {
      fetch(`/api/tasks?limit=100&poster=${address}`)
        .then(r => r.json())
  ...
  const handleRunAgain = useCallback(async (taskId: string) => {
    setRunningAgain(taskId);
    try {
      await fetch('/api/tasks/reset', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ task_id: taskId, poster_address: address }),
      });
                    ...
                    <div>
                      ...
                      <div className="flex flex-col gap-px border border-acid/8 overflow-hidden">
                        {myCompleted.map(task => (
                          <TaskRow key={task.id} task={task}
                            onClick={() => router.push(`/market/${task.id}`)}
                            onRunAgain={() => handleRunAgain(task.id)}
                            runningAgain={runningAgain === task.id}
                          />
                        ))}
                      </div>
                    </div>
...
function TaskRow({ task, onClick, onRunAgain, runningAgain }: {
  task: Task; onClick: () => void;
  onRunAgain: (() => void) | null; runningAgain: boolean;
}) {
    ...
    <div className="flex items-center gap-4 px-5 py-4 bg-black hover:bg-dim cursor-pointer transition-colors group"
      ...
      {task.status === 'completed' && (
        <div className="flex items-center gap-1.5 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
          ...
          {onRunAgain && (
            <button onClick={onRunAgain} disabled={runningAgain}
              className="flex items-center gap-1 font-mono text-xs px-2 py-1 border border-acid/20 text-muted/50 hover:text-acid hover:border-acid/40 transition-colors disabled:opacity-40"
              title="Run again">
              <RotateCcw size={9} className={runningAgain ? 'animate-spin' : ''} />
              {runningAgain ? 'RESETTING…' : 'RUN AGAIN'}
            </button>
          )}
        </div>
      )}
