...
import { Flame, Trophy, Clock, ChevronDown, ChevronUp, RotateCcw, Share2, ExternalLink, X } from 'lucide-react';
    ...
    <div className={`sub-card ${isWinner ? 'sub-winner' : ''}`} style={{ animationDelay: `${rank * 60}ms` }}>
      ...
      {expanded && (
        <div className="sub-body">
          ...
          <pre className="sub-content">{sub.content}</pre>
        </div>
      )}
    </div>
  );
...
// ─── Results View ─────────────────────────────────────────────────────────────
function ResultsView({ task, subs, onRunAgain }: { task: Task; subs: Submission[]; onRunAgain: () => void }) {
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div className="ml-auto flex items-center gap-2 flex-wrap">
          ...
          <button onClick={onRunAgain}
            className="flex items-center gap-1.5 font-mono text-xs px-3 py-1.5 border border-acid/15 text-muted hover:text-white hover:border-acid/30 transition-colors">
    ...
    <div className="standby-view">
      {!error ? (
        <div className="autostart-banner">
          <div className="autostart-spinner">
            {[0,1,2].map(i => <div key={i} className="autostart-dot" style={{ animationDelay: `${i * 0.2}s` }} />)}
          </div>
          <div>
            <p className="autostart-title">STARTING AUTOMATICALLY</p>
            <p className="autostart-desc">Agents will begin competing in a moment…</p>
          </div>
        </div>
      ) : (
  ...
  useEffect(() => {
    ...
    fetchTask().then(result => {
      ...
      if (status === 'open' && !hasAutoRun.current) {
        const alreadyRan = typeof sessionStorage !== 'undefined' && sessionStorage.getItem(autoRunKey);
        if (!alreadyRan) {
          hasAutoRun.current = true;
          if (typeof sessionStorage !== 'undefined') sessionStorage.setItem(autoRunKey, '1');
          addLog('Status is open — triggering auto-run');
          triggerRunRef.current();
        } else {
          addLog('Auto-run already fired this session — skipping');
        }
      ...
      <div className="detail-page">
        ...
        <main>
          ...
          {task.status === 'completed' && (
            <ResultsView task={task} subs={subs} onRunAgain={runAgain} />
          )}
        </main>
      </div>
    </>
